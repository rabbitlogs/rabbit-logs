...
CPIM = {
    "ko": dict(banner="CPIM", sub="생산·재고관리 국제 자격",
        ...
        left=("CPIM", "프로세스의 원리",
              ["공급망 관리의 글로벌 표준", "벤더 중립적 (시스템 무관)",
               "Oracle · Dynamics에도 적용", "\"요리의 원리\""], "Why", "왜 이 프로세스가 필요한가"),
        right=("SAP 자격증", "시스템의 기능",
               ["특정 시스템(SAP) 설정 · 구현", "벤더 종속적 (SAP 생태계 내)",
                "T-code · 설정 값 중심", "\"오븐 사용법\""], "How", "어떻게 시스템을 구현하나"),
        ),
    "en": dict(banner="CPIM", sub="an international production and inventory credential",
        ...
        left=("CPIM", "The principle behind the process",
              ["Global supply-chain standard", "Vendor-neutral (system-agnostic)",
               "Applies to Oracle, Dynamics too", "\"The principles of cooking\""],
              "Why", "why the process must exist"),
        right=("SAP certification", "The function of the system",
               ["Configure and build one system", "Vendor-specific (SAP ecosystem)",
                "Centred on T-codes and settings", "\"How to use the oven\""],
               "How", "how to build the system"),
        ),
}
    ...
    "ko": dict(banner="표준 T-CODE vs Z코드(CBO)", sub="편의성과 통제, 그 사이의 줄다리기",
        ...
        left=("표준 T-CODE", "SAP 기본 제공",
              ["강력하고 범용적", "회사 규칙은 자동 반영 안 됨",
               "사용자마다 다르게 쓰면 혼란", "통제 · 관리가 어려움"], "만능 도구"),
        right=("Z코드 (CBO)", "Customer Bolt-On",
               ["회사가 직접 추가 개발", "이름은 Z · Y로 시작",
                "회사 규칙이 이미 반영됨", "쌓이면 블랙박스화 위험"], "맞춤 비서")),
    "en": dict(banner="Standard T-codes vs Z-codes (CBO)", sub="convenience against control",
        ...
        left=("Standard T-code", "Shipped by SAP",
              ["Powerful and general-purpose", "Your rules are not built in",
               "Different habits cause confusion", "Harder to control and govern"], "All-rounder"),
        right=("Z-code (CBO)", "Customer Bolt-On",
               ["Built by your own company", "Names begin with Z or Y",
                "Company rules already embedded", "Piles up into a black box"], "Tailored aide")),
}
...
PLANNED_VS_PRODUCTION = {
    ...
    "en": dict(banner="Planned order vs production order", sub="from draft to firm instruction",
        ...
        left=("Planned order", "Created by MRP",
              ...
              "A working draft", "change it as much as you like"),
        right=("Production order", "The firm instruction",
               ...
               "A firm work order", "once fixed, execution begins"),
        ),
}
